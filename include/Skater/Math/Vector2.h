...
namespace Skater {
    struct Vector2 {
        ...
        Vector2() : Vector2(0, 0) {}
        explicit Vector2(const float val) : Vector2(val, val) {}
        ...
        [[nodiscard]] float DistanceSquared(const Vector2 rhs) const { return DistanceSquared(*this, rhs); }

        static float Distance(const Vector2 lhs, const Vector2 rhs) {
            const float v1 = lhs.X - rhs.X, v2 = lhs.Y - rhs.Y;
            return std::sqrt(v1 * v1 + v2 * v2);
        }
        [[nodiscard]] float Distance(const Vector2 rhs) const { return Distance(*this, rhs); }

        static float Dot(const Vector2 lhs, const Vector2 rhs) {
            return (lhs.X * rhs.X) + lhs.Y * rhs.Y;
        }
        [[nodiscard]] float Dot(const Vector2 rhs) const { return Dot(*this, rhs); }
        ...
        [[nodiscard]] Vector2 Lerp(const Vector2 rhs, const float t) const {
    ...
    inline const Vector2 Vector2::Zero(0);
    inline const Vector2 Vector2::One(1);
}
